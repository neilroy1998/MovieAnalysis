...
while 1==1:
	...
	out = str(check_output('ffmpeg -y -i "'+filevid+'" -ss '+start+' -t '+dur+' -q:a 0 -map a '+fileout,shell=True))

	print("\nConverting to .WAV\n")

	fileoutWAV = fileout.split(".")[0]+'.wav"'
	out = str(check_output('ffmpeg -y -i '+fileout+' '+fileoutWAV,shell=True))
	...
	with open('ran_audio.csv',mode='a',newline='') as file:
